import {
  engine,
  Transform,
} from '@dcl/sdk/ecs'
import { Color4 } from '@dcl/sdk/math'
import ReactEcs, { Button, Label, ReactEcsRenderer, UiEntity } from '@dcl/sdk/react-ecs'
import { Cube } from './components'
import { createCube } from './factory'
import { setupFlyingDemo, toggleFlyingState } from './fly'
//import { flying } from './fly'


//let flyingUI = flying



export function setupUi() {
  ReactEcsRenderer.setUiRenderer(uiComponent)
}

const uiComponent = () => (
  <UiEntity
    uiTransform={{
      width: 400,
      height: 230,
      margin: '16px 0 8px 270px',
      padding: 4,
    }}
    uiBackground={{ color: Color4.create(0, 0, 0, 0.2) }}
  >
    <UiEntity
      uiTransform={{
        width: '100%',
        height: '100%',
        flexDirection: 'column',
        alignItems: 'center',
        justifyContent: 'space-between'
      }}
      uiBackground={{ color: Color4.fromHexString("#000000") }}
    >
      <UiEntity
        uiTransform={{
          width: '100%',
          height: 80,
          margin: '8px 0'
        }}
        uiBackground={{
          textureMode: 'stretch',
          texture: {
            src: 'thumbnail.png',
          },
        }}
        uiText={{ value: 'SDK7 Smart Wearable', fontSize: 18 }}
      />
      <Label
        onMouseDown={() => {console.log('Player Position clicked !')}}
        value={`Position: ${getPlayerPosition()}`}
        fontSize={14}
        uiTransform={{ width: '100%', height: 60 } }
      ///>
      //<Label
        //onMouseDown={() => {console.log('# Cubes clicked !')}}
        //value={`# Cubes: ${[...engine.getEntitiesWith(Cube)].length}`}
        //fontSize={18}
        //uiTransform={{ width: '100%', height: 30 } }
      />
      <Button
        uiTransform={{ width: 100, height: 40, margin: 8 }}
        value='Fly'
        variant='primary'
        fontSize={14}
        onMouseDown={() => {
         // flyingUI = !flyingUI
         toggleFlyingState()
         setupFlyingDemo()
         console.log("toggle fly mode")
          
        }}
      />
        <Button
    uiTransform={{ width: 100, height: 40, margin: 8 }}
    value='Exit Fly'
    variant='primary'
    fontSize={14}
    onMouseDown={() => {
      toggleFlyingState();
      //exitFlyMode();
      console.log("exit fly mode");
    }}
 // />
 // <Button
 //   uiTransform={{ width: 100, height: 40, margin: 8 }}
 //   value='Hover at 50m'
 //   variant='primary'
 //   fontSize={14}
 //   onMouseDown={() => {
 //     toggleHoveringState();
 //     console.log("toggle hovering mode");
 //   }}
  />
     </UiEntity>
  </UiEntity>
)

function getPlayerPosition() {
  const playerPosition = Transform.getMutable(engine.PlayerEntity)
  if (!playerPosition) return ' no data yet'
  const { x, y, z } = playerPosition.position
  return `{X: ${x.toFixed(2)}, Y: ${y.toFixed(2)}, z: ${z.toFixed(2)} }`
}

